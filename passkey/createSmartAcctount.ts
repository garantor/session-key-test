...
export async function createSmartAccountWithSessionKey(passkeyValidator: any) {
  // this will generate a new session key entirely
  ...
  let seri = await serializePermissionAccount(
    sessionKeyAccount,
    sessionPrivateKey
  );
...
export async function createAccountClientWithPassKey() {
  // this will create the session key instance from a private key ready to start sending transaction
  const privateAcct = privateKeyToAccount(
